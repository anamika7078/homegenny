...
function CreateCustomerDrawer({ open, onClose, onCreated }: CreateDrawerProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.phone.trim())         { setError('Phone number is required (needed to create their login)'); return; }

    const phoneRe = /^\+?[0-9]{10,15}$/;
    if (!phoneRe.test(form.phone.trim())) {
      setError('Invalid phone — must be 10-15 digits');
      return;
    }
    ...
    if (!form.pincode.trim())       { setError('Postal / PIN Code is required'); return; }
  ...
  const autoUnitCode = deriveUnitCode(form.customer_name);

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={handleClose} />

      <div className="relative w-full max-w-[680px] bg-[#0b1120] border border-white/10 rounded-2xl flex flex-col shadow-2xl max-h-[92vh] overflow-hidden">
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/8 bg-gradient-to-r from-[#0b1120] to-[#111e38] flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">Finance module · Structured customer master details</p>
          </div>
          <button
            onClick={handleClose}
        ...
        {success ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-6 px-8 py-10 overflow-y-auto">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-slate-400">Customer master record and unique Unit Code generated.</p>
            </div>

            <div className="w-full rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5 space-y-2.5 text-xs text-slate-300">
              <div className="flex justify-between border-b border-white/5 pb-2">
                <span className="text-slate-400">Customer Name:</span>
                <span className="font-bold text-white">{success.customer_name}</span>
              </div>
              <div className="flex justify-between border-b border-white/5 pb-2">
                <span className="text-slate-400">Unique Unit Code:</span>
                <span className="font-mono text-emerald-300 font-bold px-2 py-0.5 rounded bg-emerald-500/20 border border-emerald-500/30">
                  {success.unit_code}
                </span>
              </div>
              <div className="flex justify-between border-b border-white/5 pb-2">
                <span className="text-slate-400">PAN Card:</span>
                <span className="font-mono text-slate-200 font-bold">{success.pan_card}</span>
              </div>
              {success.gstn && (
                <div className="flex justify-between border-b border-white/5 pb-2">
                  <span className="text-slate-400">GSTN:</span>
                  <span className="font-mono text-slate-200">{success.gstn}</span>
                </div>
              )}
              <div className="flex justify-between">
                <span className="text-slate-400">Address:</span>
                <span className="text-white truncate max-w-[320px]">{success.address}</span>
              </div>
              {createdPhone && (
                <div className="flex justify-between pt-2 border-t border-white/5">
                  <span className="text-slate-400">Login Access:</span>
                  <span className="text-white text-right">
                    {createdPhone} <span className="text-slate-500">/ default password</span>{' '}
                    <span className="font-mono text-emerald-300">HomeGenny@2024</span>
                  </span>
                </div>
              )}
            </div>
          ...
          <form onSubmit={handleSubmit} className="flex-1 flex flex-col min-h-0 overflow-hidden">
            <div className="flex-1 px-6 py-6 space-y-6 overflow-y-auto min-h-0 custom-scrollbar">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="w-full rounded-xl border border-emerald-500/20 bg-emerald-500/5 px-3.5 py-2.5 text-sm text-emerald-300 uppercase font-mono font-bold tracking-widest flex items-center justify-between">
                      <span>{autoUnitCode || '—'}</span>
                      <span className="text-xs text-emerald-400/60 font-sans font-normal flex items-center gap-1">🔒 Auto</span>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full rounded-xl border border-white/10 bg-[#0E1320] pl-10 pr-4 py-2.5 text-sm text-white placeholder:text-slate-600 focus:border-emerald-500/50 focus:outline-none focus:ring-1 focus:ring-emerald-500/30 font-mono tracking-widest"
                      />
                    </div>
                  </div>
                  ...
                  <div className="sm:col-span-2">
                    <p className="text-[11px] text-slate-500">
                      Creates a CLIENT login for this customer with the default password{' '}
                      <span className="font-mono text-slate-400">HomeGenny@2024</span> — they&apos;ll be asked to
              ...
              {/* ── Address Details Section (Conventional Layout) ── */}
              <div className="border-t border-white/5 pt-5 space-y-4">
                <h3 className="text-xs uppercase font-bold tracking-wider text-emerald-400 flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      placeholder="Landmark, floor, wing, additional detail"
                      className="w-full rounded-xl border border-white/10 bg-[#0E1320] px-4 py-2.5 text-sm text-white placeholder:text-slate-600 focus:border-emerald-500/50 focus:outline-none focus:ring-1 focus:ring-emerald-500/30"
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-400 uppercase tracking-widest mb-1.5">
                      Postal / ZIP Code (PIN) <span className="text-red-400">*</span>
                    </label>
                    <input
                      id="pincode"
                      name="pincode"
                      ...
                      className="w-full rounded-xl border border-white/10 bg-[#0E1320] px-4 py-2.5 text-sm text-white font-mono placeholder:text-slate-600 focus:border-emerald-500/50 focus:outline-none focus:ring-1 focus:ring-emerald-500/30 font-bold tracking-wider"
                    />
                  </div>
            ...
            <div className="px-6 py-4 border-t border-white/8 flex gap-3 flex-shrink-0">
              ...
              >
                ...
                {saving ? 'Saving Customer…' : 'Create Customer'}
              </button>
            </div>
          </form>
        )}
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">
            Create and manage customers · Unique Unit Codes & billing records
          </p>
        </div>
        <div className="flex items-center gap-3">
          <button
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/8">
                  {['Customer Name', 'Unit Code', 'PAN Card', 'Address', 'Primary GSTN', 'Bill Prefix', 'Status', ''].map(
                    (h) => (
                      <th
                        key={h}
                ...
                {filtered.map((c, i) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span className="px-2 py-0.5 rounded-md border border-indigo-500/30 bg-indigo-500/10 text-[11px] font-bold text-indigo-300 font-mono">
                        {c.unit_code}
                      </span>
                    </td>
                    <td className="px-4 py-3 font-mono text-slate-300">{c.pan_card}</td>
                    <td className="px-4 py-3 text-slate-300 max-w-[200px] truncate text-xs">
                      {c.address}
                    </td>
